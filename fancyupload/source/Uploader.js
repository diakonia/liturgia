...
var Uploader = new Class(
{
    ...
    start: function()
    {
        ...
        {
            if (this.fileList[i].status != Uploader.STATUS_QUEUED)
                continue;
            this.fileList[i].start();
            if (queued && this.uploading >= queued)
                break;
        }
        return this;
    },
        ...
        if (merged.data)
        {
            if (merged.mergeData && base.data && options.data)
            {
                if (typeOf(base.data) == 'string')
                    ...
                    merged.data = $merge(base.data, options.data);
            }
            ...
            if (query.length)
            {
                ...
                {
                    if (data.length)
                        merged.url += ((merged.url.contains('?')) ? '&' : '?') + query;
                }
                else
                {
